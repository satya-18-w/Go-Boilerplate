...
func (c *CommentRepository) AddComment(ctx context.Context, userID string, todoID uuid.UUID, payload *comment.AddCommentPayload) (*comment.Comment, error) {
	stmt := `
	INSERT INTO todo_comments(
	     todo_id,user_id,content)
		 VALUES (
		 @todo_id,
		 @user_id,
		 @content)
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to execute add comment query to todo_comment for todo_id=%s user_id=%s : %w", todoID, userID, err)
	}

	comment, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[comment.Comment])
	if err != nil {
		return nil, fmt.Errorf("Failed to fetch the row from table todo_comment  query for todo_id=%s user_id=%s: %w", todoID.String(), userID, err)
