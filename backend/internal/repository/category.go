...
func (c *CategoryRepository) CreateCategory(ctx context.Context, userID string, payload *category.CreateCategoryPayload) (*category.Category, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to execute category query for user_id = %s name = %s : %w", userID, payload.Name, err)
	}
