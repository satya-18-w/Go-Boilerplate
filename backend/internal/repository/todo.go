...
func (r *TodoRepository) GetTodoByID(ctx context.Context, userID string, todoID uuid.UUID) (*todo.PopulatedTodo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to execute get todo byid query todo_id=%s user_id=%s: %w", todoID.String(), userID, err)

	}

	todoItem, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[todo.PopulatedTodo])
	if err != nil {
		return nil, fmt.Errorf("Failed to collect row from table: todos for todo_id=%s user_is=%s : %w", todoID.String(), userID, err)
...
func (r *TodoRepository) CheckTodoExists(ctx context.Context, userId string, todoId uuid.UUID) (*todo.Todo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to execute the query to check a paricular todo exists with todo_id=%s User_id=%s : %w ", todoId.String(), userId, err)

	}

	todoItem, err := pgx.CollectOneRow(row, pgx.RowToStructByName[todo.Todo])
	if err != nil {
...
func (r *TodoRepository) GetTodos(ctx context.Context, userId string, query *todo.GetTodoQuery) (*model.PaginatedResponse[todo.PopulatedTodo], error) {
	stmt := `
	SELECT
		t.*,
		CASE
...
`
	...
	todoItem, err := pgx.CollectRows(rows, pgx.RowToStructByName[todo.PopulatedTodo])
	if err != nil {
...
func (r *TodoRepository) UpdateTodo(ctx context.Context, userID string, payload *todo.UpdateTodoPayload) (*todo.Todo, error) {
	...
	if payload.Status != nil {
		...
		// Autoset completed when status changes to completed
...
func (r *TodoRepository) DeleteTodo(ctx context.Context, userID string, payload *todo.DeleteTodoPayload) error {
	...
	args := pgx.NamedArgs{"id": payload.Id,
		"user_id": userID}

	res, err := r.server.DB.Pool.Exec(ctx, stmt, args)
	if err != nil {
		return fmt.Errorf("Failed to Execute the query : %w", err)
	}
	if res.RowsAffected() == 0 {
		code := "TODO_NOT_FOUND"
		return errs.NewBadRequestError("todo not found", false, &code, nil, nil)
	}
...
func (r *TodoRepository) GetTodoStats(ctx context.Context, userId string) (*todo.TodoStats, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to collect rowfrom table todo : %w", err)
	}

	return &stats, nil
}

// Attachment part i will do it later

func (r *TodoRepository) GetCompletedTodoOlderThan(ctx context.Context, cutoffDate time.Time, limit int) ([]todo.Todo, error) {
	stmt := `
	SELECT * FROM todos
	WHERE 
	  status='completed'
	...
	rows, err := r.server.DB.Pool.Query(ctx, stmt, args)
	if err != nil {
		return nil, fmt.Errorf("Failed to execute get completed todods older than %s query : %w", cutoffDate.Format("2006-01-02"), err)
	}
	todoItem, err := pgx.CollectRows(rows, pgx.RowToStructByName[todo.Todo])
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
...
func (r *TodoRepository) GetTodosDueInHours(ctx context.Context, hours int, limit int) ([]todo.Todo, error) {
	...
	LIMIT 
	   ...
	   `
	stmt = fmt.Sprintf(stmt, hours, limit)
	...
	if err != nil {
		...
		return nil, fmt.Errorf("Failed to fetch the rows from the Table todos : %w", err)
...
func (r *TodoRepository) ArchivedTodos(ctx context.Context, todoIDs []uuid.UUID) error {
	stmt := `
	UPDATE todos
	SET status = 'archived'
	WHERE 
...
func (r *TodoRepository) GetWeeklyStatsForUsers(ctx context.Context, startDate, endDate time.Time) ([]todo.UserWeeklyStats, error) {
	...
	weeklytodo, err := pgx.CollectRows(rows, pgx.RowToStructByName[todo.UserWeeklyStats])
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
