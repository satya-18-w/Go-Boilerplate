...
import (
	...
	testing_pkg "github.com/satya-18-w/go-TODO_TASKER/internal/testing"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func TestTodoRepository_CreateTodo(t *testing.T) {
	_, server, cleanup := testing_pkg.SetupTest(t)
	defer cleanup()
	ctx := context.Background()
	Todorepo := repository.NewTodoRepository(server)
	...
	t.Run("create Todo Successfully", func(t *testing.T) {
		...
		todos, err := Todorepo.CreateTodo(ctx, userID, &payload)
		require.NoError(t, err)
		require.NotNil(t, todos)

		assert.NotEqual(t, uuid.Nil, todos.ID)
		assert.Equal(t, userID, todos.UserID)
		assert.Equal(t, payload.Title, todos.Title)
		assert.Equal(t, payload.Description, todos.Description)
		assert.Equal(t, *payload.Priority, todos.Priority)
		assert.Equal(t, payload.DueDate.Unix(), todos.DueDate.Unix())
		assert.Equal(t, todo.StatusDraft, todos.Status)
		assert.Nil(t, todos.CompletedAt)
		testing_pkg.AssertTimestampsValid(t, todos)
...
func TestTodoRepository_DeleteTodo(t *testing.T) {
	...
	t.Run("delete todo successfully", func(t *testing.T) {
		err := todoRepo.DeleteTodo(ctx, userID, &todo.DeleteTodoPayload{Id: testTodo.ID})
		require.NoError(t, err)
	...
	t.Run("delete non-existent todo", func(t *testing.T) {
		...
		err := todoRepo.DeleteTodo(ctx, userID, &todo.DeleteTodoPayload{Id: nonExistentID})
		assert.Error(t, err)
	...
	t.Run("with canceled context", func(t *testing.T) {
		...
		err := todoRepo.DeleteTodo(canceledCtx, userID, &todo.DeleteTodoPayload{Id: testTodo.ID})
		assert.Error(t, err)
	})
}
