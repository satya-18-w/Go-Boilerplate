...
type ReminderEmailTask struct {
	...
	TaskType  string    `json:"task_type"` // duedate reminder or overdue_notification
}
...
func EnqueueReminderEmail(client *asynq.Client, task *ReminderEmailTask) error {
	...
	asynqTask := asynq.NewTask(TaskReminderEmail, payload, asynq.MaxRetry(3), asynq.Queue("default"), asynq.Timeout(30*time.Second))
