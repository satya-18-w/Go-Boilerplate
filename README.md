...
```
backend/
├── cmd/go-TODO_TASKER/        # Application entry point
...
Configuration is managed through environment variables with the `TODO_TASKER_` prefix:
...
After creating the jwt template and user in clerk paste this in the browser developer console: `await window.Clerk.session.getToken({ template: "testing-template" });`